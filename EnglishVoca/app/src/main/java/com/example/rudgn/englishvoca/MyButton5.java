package com.example.rudgn.englishvoca;  //토플, 텝스, 공무원 심화 단어 공부
...
public class MyButton5 {
    ...
    public MyButton5(int x, int y, int z, int kind) {
        this.x = x;
        this.y = y;
        this.whichPic = z;
        ...
        if(kind==1)
            for (int i = 0; i < 2; i++) {
                buttonImage[i] = BitmapFactory.decodeResource(StudyView5.mContext.getResources(), R.drawable.toefl00 + whichPic * 2 + i);

                //이전, 다음, 단어선택, 내노트, 랜덤, 나가기 버튼 및 단어선택에 나오는 8개 버튼들
                if (whichPic <60) {  //15-22 submenu
                    int xWidth = StudyView5.Width / 11;
                    int yWidth = xWidth;
        ...
        if(kind==0)
            for (int i = 0; i < 2; i++) {
                buttonImage[i] = BitmapFactory.decodeResource(StudyView5.mContext.getResources(), R.drawable.word00 + whichPic * 2 + i);
                ...
                if (whichPic == 12 || whichPic == 13 || whichPic == 33 || whichPic == 23) {
                    int xWidth = StudyView5.Width / 5;
                    int yWidth = StudyView5.Height / 7;
                ...
                if (whichPic >= 7 && whichPic <= 10) {
                    int xWidth = StudyView5.Width / 16;
                    int yWidth = xWidth;
                    buttonImage[i] = Bitmap.createScaledBitmap(buttonImage[i], xWidth, yWidth, true);
                ...
                //해설버튼
                if (whichPic == 31) {
                    int xWidth = StudyView5.Width / 13;
                    int yWidth = xWidth;
                    buttonImage[i] = Bitmap.createScaledBitmap(buttonImage[i], xWidth, yWidth, true);
                }
