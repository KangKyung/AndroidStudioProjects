package com.example.rudgn.englishvoca; //공무원 단어 공부
...
public class MyButton4 {
    ...
    public MyButton4(int x, int y, int z, int kind) {
        this.x = x;
        this.y = y;
        this.whichPic = z;
        ...
        if(kind==1)
            for (int i = 0; i < 2; i++) {
                buttonImage[i] = BitmapFactory.decodeResource(StudyView4.mContext.getResources(), R.drawable.official00 + whichPic * 2 + i);


                if (whichPic <60) {
                    int xWidth = StudyView4.Width / 11;
                    int yWidth = xWidth;
        ...
        if(kind==0)
            for (int i = 0; i < 2; i++) {
                buttonImage[i] = BitmapFactory.decodeResource(StudyView4.mContext.getResources(), R.drawable.word00 + whichPic * 2 + i);

                //이전, 다음, 단어선택, 내노트, 랜덤, 나가기 버튼
                if (whichPic < 7 ) {
                    int xWidth = StudyView4.Width / 11;
                    int yWidth = xWidth;
                ...
                if (whichPic == 12 || whichPic == 13 || whichPic == 33 || whichPic == 23) {
                    int xWidth = StudyView4.Width / 5;
                    int yWidth = StudyView4.Height / 7;
                ...
                if (whichPic >= 7 && whichPic <= 10) {
                    int xWidth = StudyView4.Width / 16;
                    int yWidth = xWidth;
                    buttonImage[i] = Bitmap.createScaledBitmap(buttonImage[i], xWidth, yWidth, true);
                ...
                //해설버튼
                if (whichPic == 31) {
                    int xWidth = StudyView4.Width / 13;
                    int yWidth = xWidth;
                    buttonImage[i] = Bitmap.createScaledBitmap(buttonImage[i], xWidth, yWidth, true);
                }
